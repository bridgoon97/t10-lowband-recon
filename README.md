...
|-----|-------------|--------|--------|-----------|
| A | DDSP harmonic+noise (oscillator synthesis) | 17.9 K | 2.69 M | Hardest to get right; F0-critical; envelope-expressivity limit (see known_issues C3) |
| B | CRN (grouped conv + GRU spectral regression) | 34.5 K | 3.08 M | Solid baseline |
| C | F-T LSTM (freq→time LSTM, FT-JNF XS style) | 13.1 K | 80.3 M | Param-efficient, MAC-expensive (exceeds 60 M budget) |
...
Vibravox body-conduction).  `python verify.py` → L0: 24/24, L1: 7/7.
...
| §5.6 STFT roundtrip | COLA + torch pair AND the causal pair used in training (catches istft/center mismatch — review C) |
...
- `reports/l1_characterization.md` — **all L1 sensor findings in one place**
  (bandwise MSC/SNR, F0 error by gender, GCC-PHAT delay, MSC variance
  decomposition) — each with sample size + boundary conditions
