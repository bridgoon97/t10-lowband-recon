...
可选：`--strength 0..1`（缩放最终修正量，默认 1）、`--mode legacy_multiply`（旧四因子乘积对照；默认 `mvp`）。输出 `Y.wav` 16 kHz PCM_16；`strength>0` 时末尾 20 ms（320 采样，共享因果 iSTFT 的病态边界区，legacy 同样存在）淡出到零以防端点爆音，其余采样不动；**`strength=0` 不做尾部淡出——全长（含尾部）等于 stage-2 输入，残差仅 PCM_16 量化（~3e-5）**。
