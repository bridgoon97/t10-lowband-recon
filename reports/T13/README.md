...
> ⚠️ **本节初版「`w_band`(MSC)坍塌、是错的度量、用不了完美幅度」的机制归因已被 A9 撤销**——带内 w_band=0.865(比 V_real 还高)，并未坍；那个 0 来自带外 bin 的全带中位伪影。**以 A9 为准：真机制是「四因子相乘」这个结构(见 A9-2)**，不在任何单一因子，也不在 MSC。
...
**唯一工作的格 = (oracle-w + 理想 V*)**。真实决策层两列都≈0⇒决策层是约束，非 Arm A。⚠️ 初版「`w_band`(MSC)是错的度量」的归因已由 A9 撤销(带内 w_band 未坍)；真机制是**四因子 `[0,1]` 软证据连乘结构**(见 A9-2)。

> [!important] 裁决
> **决策层确实独立坏 —— 但坏在「四因子 `[0,1]` 软证据连乘」这个结构，不在任何单一因子，也不在 MSC**（A9 撤销了初版的 MSC 归因）。这是个明确且局部的架构缺陷：证据应是「一个判断修哪里/多少的信号」+「安全否决(只在不安全时压到 0)」，而非四个软分数连乘。
>
...
**去掉任何单个因子都救不回来**（最好 w_local≡1 只到 0.0207，远小于 all=1 的 0.0923）；必须四个一起去掉才到 all=1。四个各自 <1 的软分数连乘 `0.573×0.718×0.865×0.153≈0.056`，平滑后 0.034——与实测 w=0.0341 吻合。⇒ **缺陷是「把四个 [0,1] 软证据相乘」这个结构，不是任何单一因子。**
...
`w_local` 对电平极敏感（±10 dB ⇒ ~16× w_local 摆动）——这是 **BR2 早指出的「纯绝对电平检测器」问题**；但**修正电平只把 dG3rec 从 0.0057 抬到 0.0091（+3.7dB 对齐），远够不着 all=1 的 0.0923**。⇒ 电平未对齐是**注入伪影**，不是 A8 结论的成因；真缺陷仍是乘性结构。后续实现 V* 注入时须做电平对齐。

### A9-4 — `w≡1` vs oracle 的差（仅报告，不修）

`all≡1` d20 = +0.0923（raw .273）vs oracle-w ~+0.39：那是「逐 `(band,t)` 最优」相对「全局常数 1」的差。**仅报告，不修**——常数 1 是粗上界，oracle 是逐点最优。

### A9 裁决（修订后口径）

- **决策层确实独立坏——但坏在「四因子 `[0,1]` 软证据连乘」这个结构，不在任何单一因子，也不在 MSC**（A8 初版 MSC 归因已撤销）。
