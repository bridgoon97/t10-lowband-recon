...
V_real：β≈1.0（每个 in-band bin 都有内容），但逐 bin 精度差。σ_vreal(pooled)=**6.61 dB** > σ_fine=6.24 ⇒ **α_vreal = 1 − 6.61/6.24 = −0.059（负！）**——V_real 比 α=0（带均值）还差，逐 bin 噪声超过谱精细结构；逐带全负（−0.225/−0.068/−0.013/−0.214）。

⇒ V_real 坐标 ~(β=1.0, α=−0.06)。**Arm A 必须先到 α=0（带均值保真，6.24 dB 误差）才能开始改善**——当前在 α 轴负侧。

### 决策（观测前定：ΔG3rec ≥ 0.30 所需最低 (β,α)，换算 dB）

达 0.30 的格点：(β=0.75,α=1.0)→.331 需 per-bin **0.00 dB**；(β=1.0,α=0.75)→.331 需 **1.56 dB**；(β=1.0,α=1.0)→.391 需 0 dB。frontier：β≤0.5 够不着（max .223）；β=0.75 需 α=1.0（0 dB）；β=1.0 需 α≥0.75（≤1.56 dB）。

⇒ **最低 per-bin 精度达 0.30 ≈ 1.0 dB**（β=1.0, α≈0.84 → ~.353），**正卡在“直接重建干净语音”的 1 dB 边界**。任何 β<1.0 都需 α=1.0（0 dB 精确重建）。

> [!important] Arm A 可行性裁决
> 要拿到 ΔG3rec ≥ 0.30（理想 0.39 的 ~3/4），**唯一不要求 0 dB 精确重建的路径是 β=1.0（全带内覆盖）+ per-bin ≤ ~1.0–1.56 dB**——这正卡在「直接重建出干净语音」的 1 dB 难度边界。V_real 当前在 (β=1, α=−0.06)，需把逐 bin 误差从 6.61 dB 压到 ~1.0 dB（~6.5× 改善）且保 β=1.0 全覆盖。**若 Arm A 达不到 β=1.0 全覆盖 + ≤1 dB 逐 bin 精度，VPU 域重建这条路工程上不成立。** 该裁决条件依赖 A6-1b 的“杀弱语义代表真实 stage-2”，用户需在真实 stage-2 输出上验证。
