...
**四因子对照**(理想 V* d20 中位 vs A4-1 V_real)：

| 因子 | 理想 V* | V_real | |
|---|---|---|---|
| c_V | 0.577 | .33–.55 | **打开** |
| g_f0 | 0.722 | .44–.52 | **打开** |
| w_band | **0.000** | .62–.83 | **塌** |
| w_local | **0.000** | .92–1 | **塌** |

机制：`w_band`(MSC)在理想 V*(X 幅度)下，被杀 bin `|V*|>>|S|` 使 MSC 分母(`<|V*|²>`含被杀能量)爆而相位抵消分子被压⇒0；`w_local` 同理坍。V_real 幅度不含被杀能量⇒w_band 不爆(0.62–0.83)但 c_V/g 压 w 到 0.033⇒J1=0。**两路径都 J1=0，不同因子失败；无输入让四因子同时工作。**
...
**唯一工作的格 = (oracle-w + 理想 V*)**。真实决策层两列都≈0⇒决策层是约束，非 Arm A。`w_band`(MSC)是错的度量：相位绑定+幅度混淆，用不了完美幅度(完美幅度反而使它坍到0)。

> [!important] 裁决
> **决策层必须重做**：`w_band`(MSC)/`w_local` 在理想 V* 下坍为0（c_V/g 反而打开）——决策层不能使用 Arm A 提供的完美逐 bin 信息。Arm A 单独投入是浪费。要么改决策层用幅度型证据(非复杂 MSC)、要么接受这条路在工程上不成立。条件依赖 A6-1b「杀弱语义代表真实 stage-2」未验证；但 A8 的结论(决策层独立坏)不依赖 D1 语义——它在理想 V* 下直接测出，与退化模型无关。
